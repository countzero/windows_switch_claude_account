...
# Sanitize name for Windows filesystem safety
function Get-SafeName($inputName) {
    if ([string]::IsNullOrEmpty($inputName)) { throw "Name required." }
    $clean = $inputName -replace '[\\/:*?"<>|\x00-\x1F]', '_'
    if ($clean -ne $inputName) { Write-Host "Sanitized to: '$clean'" -ForegroundColor Yellow }
    return $clean
}
...
function Add-To-Profile {
    ...
    $aliasDef = "Set-Alias -Name cs -Value switch_claude_account_caller -Option AllScope"
    ...
    if (-not ((Get-Content $ProfilePath -Raw) -match [regex]::Escape($MarkerStart))) {
        ...
        Write-Host "Already installed in your profile." -ForegroundColor Yellow
    }
}
...
function Remove-From-Profile {
    ...
    Write-Host "Uninstalled. Close & reopen PowerShell to remove the alias." -ForegroundColor Red
}
...
switch ($Action) {
    ...
    "save" {
        ...
        Copy-Item $CredFile (Join-Path $CredDir ".credentials.$safeName.json") -Force
    ...
    "list" {
        $slots = Get-ChildItem $CredDir -Filter ".credentials.*.json" -ErrorAction SilentlyContinue
        if ($slots) {
    ...
    "remove" {
        ...
        Write-Host "Removed '$safeName'" -ForegroundColor Red
    }
}