...
`sca usage -Watch -Warmup` (independent of `-Auto`; combines as `-Watch -Auto -Warmup`) round-robins through every saved slot before entering the steady-state polling loop. Per slot in alphabetical order: `Invoke-SlotSwap` makes it active (writes `.credentials.json` + `~/.claude.json` `oauthAccount` + `state.active_slot`), then `Invoke-SlotPrime` POSTs a minimal billable `/v1/messages` request so Anthropic opens the slot's server-side 5h session window. Per-slot progress lands in the Status column (`priming` → real HTTP outcome: `ok` / `rate-limited` / `no-oauth` / `expired` / `unauthorized` / `error`). After the last slot, a `finally` block restores the original active slot via one more `Invoke-SlotSwap` so the user ends warmup where they started. The end-state snapshot IS the first frame of the polling loop: `Invoke-UsageWatch` sets `$snapshot = $warmingSnapshot` and `$lastPoll = [DateTime]::Now` so the loop's first iteration falls into the redraw branch (no immediate re-poll). Without `-Warmup`, the watch loop polls all slots in parallel inside `Get-UsageSnapshot` and the user sees a "Waiting for first successful `/api/oauth/usage` poll..." placeholder until that returns.

**Why billable priming** rather than v2.5.0's swap-then-probe: the 5h usage window is server-side state keyed on actual billable requests within the window, NOT on which slot is locally active. v2.5.0's claim that a local swap "primes cold-slot bucket data" was wrong; that paragraph is retracted (same pattern as v2.3.1 retracted v2.3.0). Empirically, a cold slot stays cold (`/api/oauth/usage` returns rate-limited or empty buckets) until a real `/v1/messages` request opens its 5h window. Cost: ~2 tokens per slot per warmup on the pinned Haiku model (one-token input "." + `max_tokens=1` output), fractions of a cent. The probe-after-prime step from v2.5.0 is dropped: the watch loop's first poll lands within ~1 s of warmup completion and renders real bucket data then; a redundant `/api/oauth/usage` probe per slot during warmup was halved-doubling HTTP traffic for no UX gain.

**Mechanics**: `Invoke-Reconcile` fires at the start of the warmup block so a cross-account swap landing since the last `sca` call is captured before any slot bytes are read. `Invoke-WarmAllSlots` then builds the per-slot rows directly from `Get-Slots` (filtered by `-Name`, with each row's `Sidecar` carried through for `Invoke-SlotSwap`), captures the original active slot via `Read-ScaState` + `Find-SlotByName`, and walks the rows in alphabetical order. For each row: marks Status='priming' (with the `$Script:WarmupPrimingMinMs` perceptual floor sleep), calls `Invoke-SlotSwap -Slot $row`, then `Invoke-SlotPrime -SlotPath $row.Path`. Either operation throwing collapses into Status='error' with the exception message; the loop continues with the next slot. A `finally` block restores the original active slot via one more `Invoke-SlotSwap`; restore failure logs a yellow advisory (suppressed inside the watch alt-buffer but visible via the credentials state after exit) naming the slot the user is now active on. `$Script:WarmupSpacingMs` (300 ms) between slots dodges per-IP burst limits on `/v1/messages`. `Test-ClaudeRunning` guard extended to `-Warmup` at pre-loop (matches `-Auto`'s existing guard); the round-robin window is short enough that no mid-loop re-check is needed.

**Status column vocabulary** (two warmup-specific labels; terminal labels are the regular HTTP set):

| Internal label | Rendered label | Color  | Meaning |
|----------------|----------------|--------|---------|
| `warming-up`   | `warming up`   | Yellow | Queued; not yet processed |
| `priming`      | `priming`      | Yellow | Swap-then-prime in flight for this row |
| (HTTP labels)  | `ok` / `rate-limited` / `no-oauth` / `expired` / `unauthorized` / `error` | (existing colors) | Terminal: actual prime outcome for this slot |

No `warmed` / `warmup-failed` labels — the warmup terminal IS the prime's HTTP status. Swap failures share the `error` label with prime failures; the exception message in the row's Error tail names the failing operation. `-Name` filter narrows the warmup to a single slot (matches `Get-UsageSnapshot`'s filter semantics); the restore still fires if the named slot was not the original active slot. Refused when the snapshot's `Results` array is empty.
