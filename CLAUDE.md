...
| `refreshing`   | `refreshing`   | Yellow | Swap-then-probe in flight for this row |
| (HTTP labels)  | `ok` / `rate-limited` / `no-oauth` / `expired` / `unauthorized` / `error` | (existing colors) | Terminal: actual first-poll outcome for this slot |

No `warmed` / `warmup-failed` labels — the warmup terminal IS the regular HTTP status. Swap failures share the `error` label with probe failures; the exception message in the row's Error tail names the failing operation. `-Name` filter narrows the warmup to a single slot (matches `Get-UsageSnapshot`'s filter semantics); the restore still fires if the named slot was not the original active slot. Refused when the snapshot's `Results` array is empty.
