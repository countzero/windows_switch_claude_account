...
`sca usage -Watch -Warmup` (independent of `-Auto`; combines as `-Watch -Auto -Warmup`) performs the first `/api/oauth/usage` poll serially per slot before entering the steady-state polling loop. Per-slot progress lives in the Status column; the user sees each row transition from `warming up` (queued) to `refreshing` (call in flight) to the real HTTP outcome (`ok` / `rate-limited` / `no-oauth` / `expired` / `unauthorized` / `error`). The end-state snapshot IS the first frame of the polling loop: `Invoke-UsageWatch` sets `$snapshot = $warmingSnapshot` and `$lastPoll = [DateTime]::Now` so the loop's first iteration falls into the redraw branch (no immediate re-poll, the next `/api/oauth/usage` poll fires `$Interval` seconds later). Without `-Warmup`, the watch loop polls all slots in parallel inside `Get-UsageSnapshot` and the user sees a "Waiting for first successful `/api/oauth/usage` poll..." placeholder until that returns.

**Why warmup-as-first-poll** rather than the previous `/v1/oauth/token`-only warmup: a slot whose token refresh succeeds but whose `/api/oauth/usage` call is rate-limited would have shown `warmed` (green) during warmup and then `rate-limited` (red) on the first real poll a second later. By running the actual usage call during the warmup phase, the Status column reflects the actual first-poll outcome — no fake terminal label that lies about a future the poll hasn't reached.

**Mechanics**: `Invoke-Reconcile` fires at the start of the warmup block (matching the polling loop's per-poll behavior) so a cross-account swap landing since the last `sca` call is captured before reading slot bytes. `Invoke-WarmAllSlots` then iterates the snapshot's rows in `Get-Slots` order (alphabetical), calls `Get-SlotUsage -SlotPath $row.Path` for each, and copies `Status` / `Data` / `Error` / `IsCachedFallback` onto the matching row via `Set-WarmupRowResult`. `Get-SlotUsage` already handles token-refresh (with retry on 429), expired-token detection, and cache-fallback semantics; no warmup-specific duplication. Aggregate bars (rendered above the table by `Format-UsageFrame -IncludeAggregateBars`) include only rows whose `Status='ok'` and thus grow as slots complete. The `$lastAutoFooter` latch (`[Auto] Automatic slot switching is enabled.` when `-Auto` is set) is visible during warmup; the `[Watch] Last poll at HH:mm:ss` line appears after the first polling-loop redraw. 300 ms `$Script:WarmupSpacingMs` sleep between slots dodges per-IP burst limits on `/api/oauth/usage`. Pure-network operation, never writes `~/.claude.json`, so the `Test-ClaudeRunning` guard is not extended beyond `-Auto`'s existing one.
...
| `refreshing`   | `refreshing`   | Yellow | `/api/oauth/usage` call in flight for this row |
...
No `warmed` / `warmup-failed` labels — the warmup terminal IS the regular HTTP status. No per-slot cooldown (the previous 60 s cooldown machinery / `state.last_warmup_at` / `$Script:WarmupCooldownMs` / `Get-/Set-SlotWarmupTimestamp` was dropped in v2.4.0; the cooldown protected against `/v1/oauth/token` refresh thrashing, but the new warmup calls `Get-SlotUsage` which is what the polling loop does anyway). `-Name` filter narrows the warmup to a single slot (matches `Get-UsageSnapshot`'s filter semantics). Refused when the snapshot's `Results` array is empty.
