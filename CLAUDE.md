...
**Startup warmup**: on `-Auto` entry, `Invoke-WarmAllSlots` walks every saved slot in alphabetical order and forces a `/v1/oauth/token` refresh on any slot whose access token is at or near expiry. `Update-SlotTokens` retries 429s up to `$Script:TokenRefreshRetryMax` times (default 3) with exponential backoff (`$Script:TokenRefreshRetryDelayMs`, default 2 s → 4 s), so a slot whose refresh endpoint is in its brief per-token rate-limit cooldown self-recovers inside the warmup pass instead of returning `rate-limited` on the first poll. No credentials swap — the previous swap-based design was a no-op from Anthropic's perspective (local file writes generate no server-observable event). Skipped when no slots are saved. UI: a synthetic snapshot with `Status='warming-up'` (renders `warming up`, yellow) painted inside the DEC 2026 sync envelope; per-slot progress on the `[Auto]` footer line. Per-slot 60 s cooldown stored in `state.last_warmup_at[<name>]` (`$Script:WarmupCooldownMs`); timestamp written BEFORE the refresh call so a failing slot still consumes its slot. Fresh-token shortcut: a slot whose `expiresAt` is > 60 s in the future is skipped without a network call (matches `Get-SlotUsage`'s same-60 s grace threshold). 300 ms `$Script:WarmupSpacingMs` sleep between slots dodges per-IP burst limits on `/v1/oauth/token`. No-OAuth slots (api-key-only) are surfaced and skipped.
