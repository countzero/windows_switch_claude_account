...
### Added
- `usage` action reports live 5-hour session and 7-day weekly plan-usage percentages per slot by calling Anthropic's undocumented `GET /api/oauth/usage` (the same endpoint Claude Code's own `/usage` slash command uses). Auto-refreshes expired OAuth tokens against `platform.claude.com/v1/oauth/token` in-place so the hardlink to `.credentials.json` survives. `-json` emits the raw per-slot response for scripting.
- `sca usage -watch` renders a live, self-refreshing view of the usage table (or single-slot verbose view when combined with a slot name). Redraws once per second so reset deltas (`in 2h 37m`) and the countdown footer tick visibly; re-polls the endpoint every `-interval` seconds (default **30 s**, floor **30 s** — values below get clamped up with a yellow advisory). Interactive-only: throws when combined with `-json` and refuses when `[Console]::IsOutputRedirected` is true (piped to a file). Key bindings: `q` / `Esc` / `Ctrl-C` quit cleanly; `r` / `Space` force an immediate re-poll on the next tick. On mid-loop HTTP failure the previous snapshot stays visible with an advisory appended to the footer; the hardlink-broken warning is suppressed on redraws between polls so it doesn't flash every second.
...
- `Get-UsageSnapshot` / `Format-UsageFrame` / `Format-UsageFooter` split the usage action into a pure data-gathering layer and a pure rendering layer so the one-shot path and the `-watch` loop render identical frames from identical snapshots. The watch loop (`Invoke-UsageWatch`) is the only timing-dependent piece; the rendering contract it relies on is asserted by the frame-level Pester cases.
- Pester coverage: `Invoke-UsageAction` cases (happy path, empty `{}`, null `resets_at`, no-OAuth slot, 401, timeout, expired+refresh, expired+refresh-fail, `-json` round-trip, hardlink preservation across refresh, synth row with/without saved-slot content match, no synth + no warning on hardlinked active file, synth row on fresh-install (no saved slots), verbose drill-down on synth row, bare `<active>` alias acceptance, no-synth-state throw, `-json` includes synth entry), frame-level cases (`Get-UsageSnapshot` Results/HardlinkBroken/NoSlots/synth-row shape, `Format-UsageFrame` footer placement, `Format-UsageFrame -SuppressAdvisory` behavior, `-watch -json` mutex throw, `-watch` redirected-stdout throw), filename-encoding cases (labeled second-line rendering, dedup suppression, unlabeled single-line rendering), 5 `Get-SlotProfile` cases (happy/401/timeout/no-oauth/expired+refresh), 8 `Get-SlotFileInfo` parser cases, 4 `Invoke-SaveAction` filename-encoding cases (labeled on success, unlabeled on failure, dedup on name==email, fallback to unlabeled when rename fails on NTFS-invalid email), a `Get-Slots` sidecar-cleanup + labeled-parse case, an `Invoke-SwitchAction` labeled-lookup case, an `Invoke-RemoveAction` labeled-lookup case, and 3 `Get-SafeName` paren-sanitization cases. Plus the existing `Format-ResetDelta` and `Format-ResetAbsolute` formatter suites. Total: 146 tests.
- `CLAUDE.md` documents the pinned `/api/oauth/usage` + `/api/oauth/profile` constants, the re-extraction grep recipe against `claude.exe`, the verified live response schemas, the two-bucket rendering scope, the synth-row behavior, the filename-encoding grammar and parse rule, the save-time failure modes, the `-watch` loop design (snapshot / frame / loop split, guards, interval clamping, key bindings, error handling), and why the binary's hook-input schema should not be used to parse the raw endpoint.

### Fixed
- `save` no longer aborts with a traceback when the `/api/oauth/profile` response returns an email containing NTFS-invalid characters (`<`, `>`, `|`, `:`, `*`, `?`, `"`, `\`, `/`) or when a pre-existing labeled slot file is locked by another process. The slot persists unlabeled, the hardlink from `.credentials.json` stays intact, and a yellow advisory describes the fallback — matching the existing offline-save behavior. The `[Save] Saved as '<name>'` success line no longer claims an email label that did not actually land on disk; the email suffix is gated on a `$finalEmail` value set only when the rename to the labeled form (or the dedup no-rename path) actually succeeded.
