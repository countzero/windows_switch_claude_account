...
Both warmup entry points share `Invoke-WarmAllSlots`. The standalone `sca warmup [name]` action runs one warm pass and prints the usage table (no live view); `sca usage -Watch -Warmup` (independent of `-Auto`; combines as `-Watch -Auto -Warmup`) runs the same pass as the first frame before entering the polling loop. Per saved slot in alphabetical order: `Invoke-SlotSwap` makes it active, then `Invoke-SlotActivator` runs the real Claude Code CLI (`claude -p "Hi" --safe-mode --model haiku --output-format json --no-session-persistence`) as that slot so Anthropic opens its server-side 5h session window. This is exactly the manual "switch + send one message" routine; running the real client (not a hand-rolled request) delegates the OAuth token refresh to Claude Code's own flow. `--safe-mode` keeps OAuth but disables this repo's own `CLAUDE.md` / MCP / hooks / skills (cheap, no project context). Cost: ~$0.004/slot (the base Claude Code system prompt is ~3k input tokens even in safe-mode).
