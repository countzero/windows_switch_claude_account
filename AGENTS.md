...
Per-slot progress in the Status column (watch path): `warming up` (queued) → `priming` (`claude -p` in flight) → real outcome. The end-state snapshot IS the first frame of the polling loop. `$Script:WarmupSpacingMs` (300 ms) between slots dodges per-IP burst limits. Both entry points refuse pre-loop when Claude Code is already running (the per-slot swap writes `~/.claude.json`'s `oauthAccount`, which a live Claude Code caches); the short-lived `claude -p` warmup spawns is awaited to exit before the next swap, so it never overlaps a file write. The `warmup` action additionally refuses when the `claude` binary is absent.

Swap failures share the `error` label with activation failures; the exception message in the row's Error tail names the failing operation. `-Name` narrows warmup to a single slot. Refused when no slots match.

Watch keep-warm (`Invoke-KeepWarmStep`, runs each poll after the `-Auto` step): re-warms every HTTP-ok slot whose `five_hour` window has closed (bucket missing or `resets_at` null/past; a mid-window slot carries a future `resets_at`), skipping non-ok rows and any slot re-warmed within `$Script:WarmupCooldownMin` (default 5; in-memory per-session map seeded by the startup pass, never persisted). A 5h window can only be RE-opened after it closes, so this is reactive. It re-checks `Test-ClaudeRunning` per tick (the swap writes `~/.claude.json`), reuses `Invoke-WarmAllSlots -Names <cold>` (`6>$null`), does NOT merge results into the snapshot (the next poll re-reads `/api/oauth/usage`), and latches a `[Warmup]` footer line (`Keeping all slots warm.` / `Re-warmed '<names>' at HH:mm:ss` / `Re-warm refused! Claude Code is running.` / `Re-warm failed! <message>`).
