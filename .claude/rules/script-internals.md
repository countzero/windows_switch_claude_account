...
Advisories are built by the pure helper `Format-RateLimitAdvisory` (slot names from `$Snapshot.Results`, quoted, `is`/`are` per count, `>3` collapse to `'a', 'b', 'c' and N more`) in precedence order: if any row served from cache (`HasCacheFallback`), `[Usage] '<slot>' is currently rate-limited by Anthropic; showing last known usage.`; else if any row is `rate-limited` with no data (`HasRateLimited`), `[Usage] '<slot>' is currently rate-limited by Anthropic.` Naming the slot fixes the old "transient, slot active." confusion (the throttled row is usually a non-active peer, not the `*` active slot). The no-cache line states the condition without promising recovery because the renderer is shared by one-shot `sca usage` (no "next poll") and `-Watch`; the verbose view's `Get-StatusRationale` adds `temporary API throttle (429), not a plan limit`. When both cached and no-cache rate-limited rows exist the cache branch wins and names only the cached rows. The advisory is rendered by `Format-UsageFooter -Advisory` in the footer block (Yellow, leading the DarkGray `[Monitor]` / `[Watch]` lines), NOT directly under the table, so the rate-limit signal groups with the other per-frame status lines. Long error messages can't wrap the row: the `'error'` arm prefers a short `error <code>` label when the row carries a numeric `HttpStatus` (set by `Get-SlotUsage` on any non-401/403/429 HTTP failure, e.g. `error 529`), falling back to a `Format-StatusErrorTail`-normalized tail (whitespace collapse + 60-char cap) for codeless network/timeout errors; the `'expired'` arm routes through `Format-StatusErrorTail` too.
