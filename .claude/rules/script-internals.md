...
3. *Per-slot transient `refreshing` cell during the `-Warmup` startup*. The `Get-SlotUsage` call between the `refreshing` repaint and the terminal-status repaint can return in <100 ms on a warm cache, flashing the cell unreadably. `Invoke-WarmAllSlots` `Start-Sleep`s `$Script:WarmupRefreshingMinMs` (150 ms in production, 0 in tests) BEFORE `Get-SlotUsage` so the label is visible for at least the perceptual floor. Placed before HTTP (not after) so total per-slot latency is bounded at `$WarmupRefreshingMinMs + HTTP + $WarmupSpacingMs` regardless of HTTP timing. Guarded by `if (>0)` so the test override produces zero overhead, not a degenerate `Start-Sleep 0`. Pinned by a behavioral test in `Invoke-UsageAction.Tests.ps1` that asserts the sleep fires once per slot with the configured value AND that it precedes the per-slot `Invoke-RestMethod` call.
