...
**Keep-warm (`-Watch -Warmup`, `Invoke-KeepWarmStep`)**: the `-Warmup` startup pass (`Invoke-WarmAllSlots`) only opens each slot's 5h window ONCE; without a re-warm step every window closes ~5h later and stays dark for the rest of the watch. `Invoke-KeepWarmStep` runs each poll (after the `-Auto` step, so the slot it restores to is the post-rotation active one) and re-opens closed windows. Cold = row `Status -eq 'ok'` AND `five_hour` bucket missing or its `resets_at` null/`<= now` (a mid-window slot always carries a future `resets_at`); non-ok rows are skipped so a broken/throttled slot is not billed on a doomed `claude -p`. A reactive design is forced by the API: a 5h window can only be RE-opened after it closes (an earlier message just adds to the open window without renewing `resets_at`). Cooldown: an in-memory per-session map (`$warmupTimes`, slot name → last attempt `[DateTime]`, seeded at startup, never persisted to `.sca-state.json`) gates re-warms to one per `$Script:WarmupCooldownMin` (default 5) — this only matters for the failing-warm case, since a successful warm pushes `resets_at` ~5h out and the closed-window check alone then holds the slot off. Per-tick `Test-ClaudeRunning` re-check (the swap writes `~/.claude.json`). Re-warmed rows are deliberately NOT merged back into the live snapshot; the next poll re-reads `/api/oauth/usage` (≤ `-Interval` lag, explained by the footer). Like `Invoke-AutoRotationStep`, it returns a latched footer string (rendered after the `[Auto]` line, before `[Watch]`): `Keeping all slots warm.` / `Re-warmed '<names>' at HH:mm:ss` / `Re-warm refused! Claude Code is running.` / `Re-warm failed! <message>`. Never throws (warm-path exceptions become the `Re-warm failed!` line). `Invoke-WarmAllSlots` gained `-Names [string[]]` to warm just the cold subset, reusing its swap → activate → mirror → read → restore machinery.
